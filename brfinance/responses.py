    ...
    def _parse_get_search(self, reponse_json):
        ...
        if dados:
            ...
            if not search_results_df.empty:
                ...
                # BUG FIX BY KEVIN PERGHER 2023-10-22
                #response_df = response_df.append(search_results_df)
                if not search_results_df.empty:
                    response_df = pd.concat([response_df, search_results_df], axis = 0, ignore_index = False)
    ...
    def _parse_get_reports(self, html, report):
        ...
        for ind, column in enumerate(df.columns):
            if column.strip() != "Conta" and column.strip() != "Descrição":
                # BUG FIX BY KEVIN PERGHER 2023-10-22
                #df[column] = df[column].astype(str).str.strip().str.replace(".", "", regex = True)
